...
class SupplierController extends InitController {
    ...
    public function goods(){
		...
		$res = $this->doApi('/SupplierGoodsManage/goods_all',$data,'p,pagesize,action,q,brand_id,category_id,shop_category_id,s_price,e_price,status,is_best,score_type',1);
	...
	public function goods_create(){
        ...
        if ($cateId == 0) $this->redirect('/Supplier/chooseCategory');

        //店铺ID
        $shop=M('shop')->cache(true)->where($map)->field('id,type_id')->find();
        ...
        //dump($param);
        $formConfig = [
            'action' => U('/Supplier/goods_saveCreate'),
            'gourl'  => '"' . U('/Supplier/goods') . '"',
        ];

		
		//dump($_SESSION['supplier_info']['packages']);
		
		$tmp = [1 => '金积分',2=>'现金',4 => '银积分'];
		
		foreach(session('supplier_info.packages') as $v){
			$score_types[$v] = $tmp[$v];
		}
		unset($tmp);
		...
		//返还比例
		$score_ratios = [
			(100*(C('cfg.supplier')['score_ratio1']/12))=> C('cfg.supplier')['score_ratio1'],
			(100*(C('cfg.supplier')['score_ratio2']/12))=> C('cfg.supplier')['score_ratio2'],
			(100*(C('cfg.supplier')['score_ratio3']/12))=> C('cfg.supplier')['score_ratio3'],
			(100*(C('cfg.supplier')['score_ratio4']/12))=> C('cfg.supplier')['score_ratio4']
		];
...
*/
        ...
        $form = FormGroup::getInstance($formConfig)
            ...
            ->select(['name' => 'score_type', 'title' => '交易类型', 'require' => 1, 'validate' => ['required'], 'options' => $score_types])

			->select(['name' => 'score_ratio', 'title' => '返还比例(%)', 'options' => $score_ratios])
            //->callback($is_cash, ['callback' => 'text', 'title' => '乐兑宝比例(%)', 'value' => 0, 'name' => 'score_ratio', 'require' => 1, 'validate' => ['required','rangelength' => '[1,3]']])

            //->text(['name' => 'score_ratio', 'title' => '乐兑宝比例(%)', 'validate' => ['required','rangelength' => '[1,3]']])
            ->radio(['name' => 'status', 'title' => '商品状态','value'=>2, 'options' => [ 2 => '待审核'], 'require' => 1, 'validate' => ['required']])
            ...
            ->select(['name' => 'express_tpl_id', 'title' => '运费模板', 'options' => M('express_tpl')->where($tpl_map)->getField('id,tpl_name', true), 'require' => 1, 'validate' => ['required']])
            ->select(['name' => 'package_id', 'title' => '包装模板', 'options' => M('goods_package')->where($tpl_map)->getField('id,package_name', true), 'require' => 1, 'validate' => ['required']])
            ->select(['name' => 'protection_id', 'title' => '售后模板', 'options' => M('goods_protection')->where($tpl_map)->getField('id,protection_name', true), 'require' => 1, 'validate' => ['required']])
     ...
     * 创建保存
     */
    public function goods_saveCreate() {
        if (IS_POST) {
            C('TOKEN_ON', false);
            //店铺ID
            $datas = I('post.');
            ...
            try {
                ...
                $shop=M('shop')->where(['uid' => $this->seller_id])->field('id,type_id')->find();
                $datas['shop_id']   = $shop['id'];
                if($shop['type_id']==1) $datas['is_self'] = 1;

                $datas['uptime']            = date('Y-m-d H:i:s');
                $datas['seller_id']         = $this->seller_id;
                //$datas['score_ratio']       = 1;
				...
				//检查是否到达套餐发布商品上限
				if(false === $this->checkToplimit($datas['score_type'])) throw new Exception('此支付类别商品超过发布上限');
				//session('user.id');
				//$datas['score_type']
				//$count = M('goods')->where(['seller_id'=>$this->seller_id,'supplier_id'=>session('user.id'),'score_type'=>$data['score_type']])->count();
				
				if($datas['score_type'] == 2 && !($datas['score_ratio'] > 0)){
					throw new Exception('第三方支付必须选择返还比例');
				}else if($datas['score_type'] != 2){
					$datas['score_ratio'] = 0;
				}

                //检查商家推荐商品数量上限
                if($datas['is_best'] == 1){
                    $max_best=M('shop')->where(['uid' => $this->seller_id])->getField('max_best');
                    $count=M('goods')->where(['seller_id' => $this->seller_id,'supplier_id'=>session('user.id'),'status' => 1,'num' => ['gt',0],'is_best' =>1])->count();
                    if($count+1 > $max_best) throw new Exception('橱窗推荐已经超出上限');
                }

                //是否包邮
                $datas['free_express']  =M('express_tpl')->where(['uid'=>$this->seller_id,'id'=>$datas['express_tpl_id']])->getField('is_free');
                ...
                $goodsAttrList      = $this->parseAttrs($datas['attrs'], $datas['values'], $datas['images'],$datas['score_type']); //商品属性
				...
				$data['supplier_id'] = session('user.id');
				$data['examine_status'] = 2;                                    //审核状态
                //服务承诺
...
//                if ($goodsCollocation) $data['goods_collocation']   =   $goodsCollocation;
    ...
    public function goods_edit() {
        ...
        $goods = D('GoodsEditRelation')->relation(true)->where(['seller_id' =>$this->seller_id,'id' => $id])->find();
        if ($goods['category_id'] == 100845550) {   //如果商品分类为其他分类  则需要设置分类
            //$this->redirect('/goods/editCate', ['id' => $id]);
            //exit;
        }
        //店铺ID
        $shop       = M('shop')->cache(true)->where(['uid' => $this->seller_id])->field('id,type_id')->find();
        $daigou     = getSiteConfig('daigou');
        $daigou_shop= explode(',',$daigou['daigou_goods_id']);
        $isDaigou   = false;
        if(in_array($shop['id'],$daigou_shop)){
...
//        }
        ...
        $formConfig = [
            'action' => U('/Supplier/goods_saveEdit'),
            'gourl'  => '"' . U('/Supplier/goods') . '"',
        ];
...
*/
        ...
        $form = FormGroup::getInstance($formConfig)
            ...
            ->select(['name' => 'score_type', 'title' => '交易类型', 'require' => 1, 'validate' => ['required'], 'options' => $score_types, 'value' => $goods['score_type']])
            ->select(['name' => 'score_ratio', 'title' => '返还比例(%)','value'=>$goods['score_ratio'], 'options' => $score_ratios])
            //->text(['name' => 'score_ratio', 'title' => '乐兑宝比例(%)','value'=>$goods['score_ratio'], 'validate' => ['requireed','rangelength' => '[1,3]']])
            ->radio(['name' => 'status', 'value' => 2, 'title' => '商品状态', 'options' => [2 => '待审核'], 'require' => 1, 'validate' => ['required']])
            ...
            ->select(['name' => 'express_tpl_id', 'value' => $goods['express_tpl_id'], 'title' => '运费模板', 'options' => M('express_tpl')->where($tpl_map)->getField('id,tpl_name', true), 'require' => 1, 'validate' => ['required']])
            ->select(['name' => 'package_id', 'value' => $goods['package_id'], 'title' => '包装模板', 'options' => M('goods_package')->where($tpl_map)->getField('id,package_name', true), 'require' => 1, 'validate' => ['required']])
            ->select(['name' => 'protection_id', 'value' => $goods['protection_id'], 'title' => '售后模板', 'options' => M('goods_protection')->where($tpl_map)->getField('id,protection_name', true), 'require' => 1, 'validate' => ['required']])
	...
	public function goods_saveEdit() {
        if (IS_POST) {
            C('TOKEN_ON', false);
            ...
            try {
                ...
                //是否包邮
                $datas['free_express']  = M('express_tpl')->where(['uid'=>$this->seller_id,'id'=>$datas['express_tpl_id'],'supplier_id'=>session('user.id')])->getField('is_free');
				...
				//检查是否到达套餐发布商品上限
				if(false === $this->checkToplimit($datas['score_type'],$id)) throw new Exception('此支付类别商品超过发布上限');
                //检查商家推荐商品数量上限
                if($datas['is_best'] == 1){
                    $max_best   = M('shop')->where($map)->getField('max_best');
                    $count      = M('goods')->where(['seller_id'=>$this->seller_id,'supplier_id' => session('user.id'),'status' => 1,'num' => ['gt',0],'is_best' =>1,'id'=>['neq',$datas['id']]])->count();
                    if($count+1 > $max_best) throw new Exception('橱窗推荐已经超出上限！');
                }
                ...
                $goodsContent       = ['content' => htmlentities(strip_tags(html_entity_decode($datas['content']),'<div><span><img><table><tr><td><thead><tbody><font><strong><br><b><hr>'))]; //商品详情
                $goodsAttrList      = $this->parseAttrs($datas['attrs'], $datas['values'], $datas['images'],$datas['score_type']); //商品属性/值
                $goodsParam         = $this->parseParams($datas['param']); //商品参数
                ...
                if ($goodsAttrList['attrs']) {
                    ...
                    if ($goodsAttrListIds) {    //如果不为空则删除未编辑的
                        $falg = M('goods_attr_list')->where(['seller_id' => $this->seller_id, 'goods_id' => $id, 'id' => ['not in', rtrim($goodsAttrListIds, ',')]])->delete();
    ...
    public function editCate() {
        ...
        if ($id > 0) {
            ...
            $goods = M('goods')->where(['id' => $id, 'seller_id' => $this->seller_id])->find();
            //dump($goods);
            if ($goods) {
                $this->checkCreatePermissions();
                ...
                $this->redirect('/Supplier/goods_edit', ['id' => $id]);
            }
        } else {
            $this->redirect('/goods');
        }
    ...
    public function goods_choose() {
        ...
        $res = $this->doApi('/SupplierGoodsManage/goods_online',$data,'p,pagesize,action,q,code,category_id,shop_category_id,s_price,e_price,s_sale,e_sale,is_best');
    ...
    public function getTmallItem(){
        ...
        //查询店铺ID
        $userInfo = M('user')->field('id')->where('id = "'.$this->seller_id.'"')->find();
        ...
        $data['express_tpl_id']=M('express_tpl')->where($uidMap)->getField('id');
        //供货商
    ...
    private function checkCreatePermissions() {
        ...
        //取商品类目权限
        $cates = M('shop')->where(['uid' => $this->seller_id])->field('category_id,category_second')->find();
    ...
    protected function checkShopCate($cateId){
        ...
        $cates = M('shop')->where(['id' => $this->getShopId()])->cache(true)->getField('category_second');
     ...
     * @return array|bool
     */
    public function parseAttrs($datas, $values, $images,$score_type) {
        ...
        if (!empty($datas) && is_array($datas)) {
			...
			//不是现金交易计算成本价
			if($score_type != 2){
				$tmp['data']['price']       = min($datas['price_purchase']);
				$tmp['data']['price_max']   = max($datas['price_purchase']);
			}else{
                ...
                foreach ($v as $key => $val) {
                    ...
                    foreach ($tmp['values'] as $value) {
                        if (strpos($datas['attr'][$key], $value['option_id']) !== false) {
							...
							//卖家指定商家
							$tmp['attrs'][$key]['seller_id']   	= $this->seller_id;
							//金积分乘倍数
							if($score_type == 1){
								$tmp['attrs'][$key]['price'] = $tmp['attrs'][$key]['price_purchase'] * C('cfg.supplier')['gold_multiply'];
							//银积分乘倍数
							}else if($score_type == 4){
								$tmp['attrs'][$key]['price'] = $tmp['attrs'][$key]['price_purchase'] * C('cfg.supplier')['silver_multiply'];
							}
                        }
                    }
                }
			...
			//金银积分乘倍数
			if($score_type == 1){
				$tmp['data']['price']       = $tmp['data']['price'] * C('cfg.supplier')['gold_multiply'];
				$tmp['data']['price_max']   = $tmp['data']['price_max'] * C('cfg.supplier')['gold_multiply'];
			//银积分乘倍数
			}else if($score_type == 4){
				$tmp['data']['price']       = $tmp['data']['price'] * C('cfg.supplier')['silver_multiply'];
				$tmp['data']['price_max']   = $tmp['data']['price_max'] * C('cfg.supplier')['silver_multiply'];
			}
			//writelog($tmp['data']);
...
//                ),
	...
	protected function getShopId(){
		return M('shop')->cache(true)->where(['uid' => $this->seller_id])->getField('id');
	}
     ...
     * 2017-09-13
     */
	protected function checkToplimit($score_type,$id=0){
	    $map['seller_id'] = $this->seller_id;
	    $map['supplier_id'] = session('user.id');
	    $map['score_type'] = $score_type;
	    if($id>0){
            $map['id'] = ['neq',$id];
        }
		...
		if($score_type == 1){
			$max = session('supplier_info.gold_num');
		}else if($score_type == 2){
			$max = session('supplier_info.cash_num');
		}else if($score_type == 4){
			$max = session('supplier_info.silver_num');
		}
