...
class Beanstalkd {
	...
	public function __construct(array $config = []) {
		$defaults = [
			'persistent'    => false,
			'host'          => isset($config['host']) ? $config['host'] : BS_HOST,
			'port'          => isset($config['port']) ? $config['port'] : BS_PORT,
			'timeout'       => isset($config['timeout']) ? $config['timeout'] : BS_TIMEOUT,
	...
	public function put($pri, $delay, $ttr, $data) {
		...
		$this->log = [$status, sprintf("put %d %d %d %d\r\n%s", $pri, $delay, $ttr, strlen($data), $data)];
