...
<head>
	...
	<script>
		var ACTION	=	'<?php echo enCryptRestUri("/Package/index");?>';
	</script>
</head>
    ...
    <div class="centent_body">
        ...
        <div class="revision_bg">
            <div class="container ptb_size_lg clearfix">
                <!-- 左侧导航 -->
                <div class="revisionLeft controller-active" data-controller="/Package" data-url="/package.html" style="width: 150px;">
    <div class="pb10">
        <?php $_result=getShopMenu();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($vo["position"]) == "4"): ?><div class="user_left_tle">
                ...
                <div class="revisionRight pd15" style="width: 1020px"><!-- 右侧内容 -->
                    <div class="clearfix revisionOrderTab" data-url="">
                        <a class="fr btn btn-primary" href="<?php echo U('/package/create');?>">创建模板</a>
                        <a class="orderOption active" href="javascript:;">包装模板管理</a>
                    </div>

                    <!-- 表单 -->
                    <div class="content">
                        <?php if(empty($data)): echo W('Common/Builder/nors', array('param' => array('text' => $data['msg'])));?>
                            <?php else: ?>
                            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="alert solid_b">
                                    <a href="javascript:;" data-id="<?php echo ($vo["id"]); ?>" onclick="del('<?php echo ($vo["id"]); ?>');" class="fr text_yellow fs14 del-addr">删除</a>
                                    <a href="<?php echo U('/package/create', ['id' => $vo['id']]);?>" class="fr text_blue fs14 mr10">修改</a>
                                    <strong class="fs14 mr10 <?php if(($vo["is_default"]) == "1"): ?>text_green<?php endif; ?>"><?php echo ($vo["package_name"]); ?></strong> <span class="fs14 mr10" style="color: #b8b8b8">|</span> <span style="color: #b8b8b8"><?php echo ($vo["atime"]); ?></span>
