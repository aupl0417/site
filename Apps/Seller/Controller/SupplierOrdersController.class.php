...
/*
        ...
        $this->authApi('/SupplierOrders/orders_list', $map,'sday,eday,goods_name,s_no,pagesize,nick,receipt_time,s_no')->with();
    ...
    public function detail() {
        $id =   I('get.id');
        $this->authApi('/SupplierOrders/view', ['s_no' => $id])->with();
        ...
        $this->authApi('/SupplierOrders/orders_logs', ['s_no' => $id])->with('logs');
        C('seo', ['title' => '订单详情']);
        $this->display();
    }
...
//                 }
    ...
    public function close() {
        ...
        $config = [
            ...
            'gourl'  => '"' . U('/SupplierOrders/detail', ['id' => $sno]) . '"',
        ];
        $form = Form::getInstance($config)
    ...
    public function express() {
        ...
        $form = Form::getInstance($config)
            ->hidden(['name' => 's_no', 'value' => $sno])
            ->modal(['name' => 'express_company_id', 'title' => '快递公司', 'url' => U('/SupplierTool/expressCompany', ['inputName' => 'express_company_id']), 'require' => 1, 'validate' => ['required']])
    ...
    public function editExpress() {
        ...
        $this->getData($sno);
        $config = [
            'action' => U('/SupplierOrders/edit_express_save'),
            'gourl'  => '"' . U('/SupplierOrders/detail', ['id' => $sno]) . '"',
        ];
        $form = Form::getInstance($config)
            ->hidden(['name' => 's_no', 'value' => $sno])
    ...
    public function edit_express_save(){
		...
		//$this->authApi('/SupplierOrders/view', ['s_no' => $data['s_no']])->with();
		//dump($this->_data);
//exit();
        $res = $this->authApi('/SupplierOrders/edit_express',$data,'express_remark');
		$res = $this->_data;
        $this->ajaxReturn($res);
    }
