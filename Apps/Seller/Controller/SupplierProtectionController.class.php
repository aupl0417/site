...
class SupplierProtectionController extends Controller {
	private $seller_id;
    ...
    public function index() {
        $data   =   M('goods_protection')->where(['uid'=>$this->seller_id,'supplier_id' => getUid()])->field('protection_name,atime,id')->order('id desc')->select();
...
*/
    ...
    public function create()
    {
        ...
        $title  =   '创建售后模板';
        if ($id > 0) {
            $data   =   M('goods_protection')->where(['id' => $id, 'uid'=>$this->seller_id,'supplier_id' => getUid()])->find();
            $data['content']    =   html_entity_decode($data['content']);
            $title = '修改售后模板';
        }

        $config = [
            'action' => U('/SupplierProtection/save'),
            'gourl'  => '"' . U('/SupplierProtection') . '"',
        ];
    ...
    public function del() {
        $id =   I('post.id');
        if ($id) {
            $flag   =   M('goods_protection')->where(['id' => ['in', $id], 'uid'=>$this->seller_id,'supplier_id' => getUid()])->delete();
