...
class SupplierExpressController extends CommonController{
	...
	*/
	public function index(){
		...
		$list=$do->relation(true)->where(['uid'=>$this->seller_id,'supplier_id'=>session('user.id')])->field('ip,etime',true)->order('id desc')->select();
	...
	*/
	public function delete(){
		...
		$count=M('goods')->where(array('uid' => $this->seller_id,'express_tpl_id' => I('post.id'),'status' => array('gt',0)))->count();
		if($count>0) $this->ajaxReturn(array('code'=>0,'msg'=>'您有个'.$count.'商品应用了该模板(含仓库中、违规、及主图异常商品)，必须解除应用后才可以删除！'));

		if($do->where(array('id' => I('post.id'),'uid'=>$this->seller_id,'seller_id' => session('user.id')))->delete()){
	...
	*/
	public function edit(){
		...
		$rs=$do->relation(true)->where(['uid'=>$this->seller_id,'supplier_id'=>session('user.id'),'id' => I('get.id')])->find();
		$area=$this->cache_table('area');
	...
	*/
	public function edit_save(){
		...
		$_POST['uid']	=$this->seller_id;
		$_POST['supplier_id']	=session('user.id');
	...
	*/
	public function area_delete(){
		$do=M('express_area');
		if($do->where(array('id' => I('post.id'),'uid'=>$this->seller_id,'supplier_id'=>session('user.id')))->delete()){
			$this->ajaxReturn(array('code'=>1,'msg'=>'删除成功！'));
		}else{
			$this->ajaxReturn(array('code'=>0,'msg'=>'删除失败！'));
		}
