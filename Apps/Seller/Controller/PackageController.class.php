...
class PackageController extends AuthController {
    ...
    public function index() {
        C('seo', ['title' => '包装模板管理']);
        $data   =   M('goods_package')->where(['uid' => getUid()])->field('package_name,atime,id')->order('id desc')->select();
    ...
    public function del() {
        $id =   I('post.id');
        if ($id) {
            $flag   =   M('goods_package')->where(['id' => ['in', $id], 'uid' => getUid()])->delete();
