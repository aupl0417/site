...
class ZhaoshangController extends CommonController {
    ...
    public function _initialize() {
        ...
        if($this->uid) {
            $this->info = M('zhaoshang_join')->where(['uid' => $this->uid])->field('atime,etime,ip',true)->find();
    ...
    public function _info(){
        $rs = M('zhaoshang_join')->where(['uid' => $this->uid])->field('atime,etime,ip',true)->find();
        if($rs){
    ...
    public function _shop_type_set(){
        ...
        if($_POST['is_edit'] !=1 && $this->info['step'] < 1) $vdata['step']   = 1;

        //print_r($vdata);

        if($id = M('zhaoshang_join')->where(['uid' => $this->uid])->getField('id')){
            if(false !== M('zhaoshang_join')->where(['id' => $id])->save($vdata)){
                return ['code' => 1,'data' => ['brand_num' => count($brand)]];
            }else return ['code' => 0];
        }elseif(M('zhaoshang_join')->add($vdata)){
            return ['code' => 1,'data' => ['brand_num' => count($brand)]];
        }else return ['code' => 0];
    ...
    public function _brand_save(){
        ...
        if($_POST['is_edit'] !=1 && $this->info['step'] < 2) $data['step']   = 2;

        if(false !== M('zhaoshang_join')->where(['uid' => $this->uid])->save($data)){
            return ['code' => 1];
        }else return ['code' => 0];
    }
    ...
    public function _industry_save(){
        ...
        if($_POST['is_edit'] !=1 && $this->info['step'] < 3) $data['step']   = 3;
    ...
    public function _shop_info_save(){
        ...
        $do = D('Common/ZhaoshangShopInfo');
        $data = I('post.');
        $data['status'] = 1;
        $data['inventory_type'] = $data['inventory_type'] ? $data['inventory_type'] : 0;
        if($_POST['is_edit'] !=1 && $this->info['step'] < 4) $data['step']   = 4;

        if(!$do->create($data)) return ['code' => 4,'msg' => $do->getError()];

        if(false !== $do->where(['uid' => $this->uid])->save($data)){
            return ['code' => 1];
        }else return ['code' => 0];
    ...
    public function _last_logs(){
        $rs = M('zhaoshang_logs')->where(['status' => ['in','2,4'],'zhaoshang_join_id' => $this->info['id']])->field('id,content,remark')->order('id desc')->find();
        if($rs){
