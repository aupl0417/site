...
class AddressController extends CommonController {
    ...
    */
    public function address_list(){
        ...
        $list=$do->where(array('uid'=>$this->uid))->field('etime,ip',true)->order('is_default desc,id desc')->select();
        if($list){
    ...
    */
    public function view(){
        ...
        $do=M('shopping_address');
        $rs=$do->cache(true,C('CACHE_LEVEL.XXS'))->where(array('uid'=>$this->uid,'id'=>I('post.id')))->field('atime,etime,ip',true)->find();
        if($rs){
            //返回详情
            $area   =$this->cache_table('area');
