...
class PaymentController extends AuthController {
    ...
    public function index(){
        ...
        $order = array(
            ...
            'goodsName'     => str_replace($this->filter_string, '', $goods_name),                         //商品名称
            'remark'        => $remark,                             //订单备注
            'returnUrl'     => DM('cart', '/DtpayReturn/return_multi'),     //同步通知地址
            'notifyUrl'     => DM('cart', '/DtpayReturn/notify_multi'),     //异步通知地址
            //'payAccountCode'=> 'TRJ_SHOPPING',  //专用通道标识
        );
		if(I('post.terminal') == 'wap'){
			$order['returnUrl'] = DM('m', '/DtpayReturn/return_multi');
		}
    ...
    public function sub_orders($s_no,$paytype){
        ...
        if($coupon) {
            if($paytype == 2) return ['code' => 0,'msg' => '该订单使用了唐人街官方优惠券不支持唐宝支付！'];
            //提交至ERP
            //C('DEBUG_API',true);
            $tmp = $this->doApi('/Erp/put_coupon',['coupon' => $coupon],'',1);
        ...
        $data = array (
            ...
            'goodsName'     => str_replace($this->filter_string, '', $goods_name),                                     //订购商品名称
            'remark'        => str_replace($this->filter_string, '', $remark),                                         //备注
            'disabledPay'   => '',
            'returnUrl'     => DM('cart', '/DtpayReturn/return_single'),        //同步通知地址
    ...
    public function single_pay(){
		...
		//wap同步返回地址
		if(I('post.terminal') == 'wap'){
			$res['data']['returnUrl'] = DM('m', '/DtpayReturn/return_single');
		}
