...
class AppealController extends OrdersController{
    ...
    private function statusName($param) {
        $statusArr      =   [];
        if ($param['orders_status'] == 2) {
            $statusArr  =   [2 => '卖家拒绝退款，买家提出申诉'];
        } elseif ($param['orders_status'] == 3) {
            $statusArr  =   [2 => '卖家拒绝退款，买家提出申诉'];
        } elseif ($param['orders_status'] > 3) {
            $statusArr  =   [2 => '卖家拒绝售后，买家提出申诉' ,5 => '卖家操作太久，买家提出申诉',6 => '买家未收到售后商品提出申诉'];
        }
        if (!array_key_exists($param['status'], $statusArr) || empty($statusArr)) return false;
        
        return '<p class="strong text_red">买家提出申诉</p>' . $statusArr[$param['status']];
    }
}