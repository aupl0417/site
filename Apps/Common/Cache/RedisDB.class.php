 ...
 */
use Vendor\Cluster\RpcLog;
use Vendor\Cluster\RpcLogEnvConfig;
use Vendor\Cluster\SentinelMonitor;
...
class RedisDB {
	...
	private function _init($group) {
		...
		if (true === empty($this->_sentinel->servers)) {
		    $sentinelConfig = C('SENTINELS');
			$this->_sentinel->addServers($sentinelConfig);
		}
		...
		switch ($method) {
			case 2:
				if (true === empty($this->_conn[$this->_group]['master'])) {
			        $sTime = RpcLog::getMicroTime();
					$host = $this->_sentinel->getMasterServer();
					//$host[0] = '192.168.16.161';
                    $this->_conn[$this->_group]['master'] = new \Redis;
					$this->_conn[$this->_group]['master']->connect($host[0], $host[1], self::TIME_OUT);
                    $eTime = RpcLog::getMicroTime();
                    if (empty($this->_conn[$this->_group]['master']->socket)) {
                        //RpcLogEnvConfig
                        RpcLog::log('connect master fail, date is ' . date('Y-m-d H:i:s', time()) . ' host ' . $host[0] . ' port ' . $host[1], $sTime, $eTime, RpcLogEnvConfig::RPC_LOG_TYPE_REDIS_CONNECT_FAIL);
                        die('connect master fail');
                    }
			...
			default:
				if (true === empty($this->_conn[$this->_group]['slave'])) {
                    ...
                    $eTime = RpcLog::getMicroTime();
                    if (empty($this->_conn[$this->_group]['slave']->socket)) {  //if current host connect fail
                        RpcLog::log('connect slave fail, date is ' . date('Y-m-d H:i:s', time()) . ' host ' . $host[0] . ' port ' . $host[1], $sTime, $eTime, RpcLogEnvConfig::RPC_LOG_TYPE_REDIS_CONNECT_FAIL);
                        $host = $this->_sentinel->getSlaveServer('', $host[0].':'.$host[1]);
                        $this->_conn[$this->_group]['slave']->connect($host[0], $host[1], self::TIME_OUT);
                    }
					$this->_servers['host'] = $host[0];
					$this->_servers['port'] = $host[1];
	...
	public function keys($key) {
        $this->_read($key);
        $result = $this->_conn[$this->_group]['salve']->keys($key);
        return $result;
    }
