...
class AuthpersonController extends CommonModulesController {
	protected $name 			='个人认证';	//控制器名称
    protected $formtpl_id		=99;			//表单模板ID
    ...
    */
    public function index($param=null){
    	...
    	$this->assign('fields',$this->plist('',$btn));
		$this->display();
    }
	...
	public function logs_add(){
		...
		if(I('post.status')==1){
			if($rs['status']!=I('post.status')){
				if(!$sw2=M('auth_person')->where(array('id'=>I('post.auth_id')))->save(array('status'=>I('post.status'),'success_time'=>date('Y-m-d H:i:s')))) goto error;
			}
			if($rs['user']['is_auth']!=1) {
				if(!$sw3=M('user')->where(array('id'=>$rs['uid']))->save(array('is_auth'=>1,'name'=>$rs['name']))) goto error;
			}
		}else{
			if($rs['status']!=I('post.status')){
				if(!$sw2=M('auth_person')->where(array('id'=>I('post.auth_id')))->save(array('status'=>I('post.status')))) goto error;
			}
			if($rs['user']['is_auth']==1) {
