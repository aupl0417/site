...
function U($url='',$vars='',$suffix=true,$domain=false) {
	...
	*/
	...
	//echo $url_key.'<br>';
	$iscmp=0;
	foreach(C('URL_CMP') as $key=>$val){
		if($key==$url_key){
			$iscmp=1;
			$url=url_cmp(array('url_rex'=>$val,'rs'=>$vars)).'.'.($suffix===true?C('URL_HTML_SUFFIX'):'');
		}
	}	
	if($iscmp==1) return $url;
...
function S($name,$value='',$options=null) {
    ...
    if(is_null($name)){
		if(C('DATA_CACHE_TYPE') == 'memcached') $cache->clearAllKeys();
        else $cache->clear();
	}elseif(''=== $value){ // 获取缓存
