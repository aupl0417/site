...
class ReadHtmlCacheBehavior {
         ...
         if(!empty($htmls)) {
            ...
            if(!empty($html)) {
                ...
                $rule  = preg_replace_callback('/{\|(\w+)}/', function($match){
                    $fun = $match[1];
                    if (in_array($fun, ['minute', 'hour'])) {   //如果有时间目录，则写入把html path写入memcache
                        $id = I('get.id', 0, 'int');
                        if ($id == 0) return false;
                        $cacheName = md5('mall_goods_html_cache_id_' . $id);
                        $tmp = S($cacheName);
                        if ($tmp == false) {
                            S($cacheName, 1);
                        }
                ...
                //如果有时间目录，则写入把html path写入memcache
                $id = I('get.id', 0, 'int');
                $cacheName = md5('mall_goods_html_cache_id_' . $id);
                $tmp = S($cacheName);
                $htmlPath = HTML_PATH . $rule.C('HTML_FILE_SUFFIX',null,'.html');
                if ($tmp == false) {
                    
                } elseif ($tmp == 1) {
                    $filectime = filectime($htmlPath);
                    S($cacheName, $htmlPath, (($filectime + 1800) - NOW_TIME));
                    unset($filectime);
                } else {
                    $htmlPath = $tmp;
                }
                unset($tmp,$cacheName,$id);
                // 当前缓存文件
                define('HTML_FILE_NAME', $htmlPath);
                return $cacheTime;
            }
        }
        // 无需缓存
     ...
     * 是否不需要读取缓存
     */
    static private function isNotCache() {
        
        //diy页面不适用缓存页面
        if (isset($_GET['diy']) && I('get.diy') == 'yes' && $_SESSION['admin']['id'] > 0) {
            return false;
        }
        
        //商品详情
        if (strpos(HTML_FILE_NAME, '/Item/') !== false) {
            $id = I('get.id', 0, 'int');
            if ($id < 1) return false;
            $data = D('Common/IsNotHtmlCacheView')->where(['id' => $id])->field('shop_id,status,goods_id,officialactivity_join_id,sku_num')->find();
            unset($attr);
            //是否有参与官方活动,库存是否大于0
            if ($data['status'] != 1 || $data['officialactivity_join_id'] > 0 || $data['sku_num'] <= 0) { //如果当前商品的状态不等于1则表示不正常
                unset($data);
                return false;
            }
            //是否有参与常规活动
            if (Activity::getSpikeAndRestriction($data['shop_id'], $data['goods_id']) || getActivityGoods($data['shop_id'])) {
                unset($data);
                return false;
            }
        }
