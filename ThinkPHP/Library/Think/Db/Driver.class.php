...
abstract class Driver {
    ...
    public function query($str,$fetchSql=false) {
        ...
        if ( false === $result ) {
			...
			$res=$this->getResult();
			if(C('ADMIN_LOG') && !in_array(substr($table,1,-1),C('LOG_NOT_RECORD_TABLE'))) admin_log($table,$this->queryStr,$res!==false?1:0,$res);
            return $res;
     ...
     * @return mixed
     */
    public function execute($str,$fetchSql=false,$table='') {
        ...
        if ( false === $result) {
			...
			if(C('ADMIN_LOG') && !in_array(substr($table,1,-1),C('LOG_NOT_RECORD_TABLE'))) admin_log($table,$this->queryStr,$this->lastInsID!==false?1:0,$this->lastInsID?$this->lastInsID:$this->numRows);
            return $this->numRows;
        }
    }
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        return $this->execute($sql,!empty($options['fetch_sql']) ? true : false,$this->parseTable($options['table']));
    }
    ...
    public function selectInsert($fields,$table,$options=array()) {
        ...
        return $this->execute($sql,!empty($options['fetch_sql']) ? true : false,$this->parseTable($table));
    }
    ...
    public function update($data,$options) {
        ...
        $sql .=   $this->parseComment(!empty($options['comment'])?$options['comment']:'');
        return $this->execute($sql,!empty($options['fetch_sql']) ? true : false,$table);
    }
    ...
    protected function debug($start) {
        if($this->config['debug']) {// 开启数据库调试模式
            if($start) {
                ...
                // 记录操作结束时间
                G('queryEndTime');
				if(C('APP_DEBUG') || C('DB_SQL_LOG')) trace($this->queryStr.' [ RunTime:'.G('queryStartTime','queryEndTime').'s ]','','SQL');				
            }
        }
    }
