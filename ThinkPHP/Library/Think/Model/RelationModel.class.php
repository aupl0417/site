...
class RelationModel extends Model {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                        ...
                        switch($mappingType) {
                            case self::HAS_ONE:
                                ...
                                $relationData   =  $model->where($mappingCondition)->field($mappingFields,$val['mapping_fields_type'])->find();
                            ...
                            case self::HAS_MANY:
                                ...
                                // 延时获取关联记录
                                if($val['is_getfield']==1){
                                    $relationData   =  $model->where($mappingCondition)->order($mappingOrder)->limit($mappingLimit)->getField($mappingFields);
                                }elseif($val['group_field']){
                                    $relationData   =  $model->where($mappingCondition)->field($mappingFields,$val['mapping_fields_type'])->group($val['group_field'])->order($mappingOrder)->limit($mappingLimit)->select();
                                }else{
                                    $relationData   =  $model->where($mappingCondition)->field($mappingFields,$val['mapping_fields_type'])->order($mappingOrder)->limit($mappingLimit)->select();
                                }
	...
	public function relationWhere($model,$where){
		$this->_link[$model]['condition']=$where;
		return $this;
	}
	
    public function relationOrder($model,$order){
        $this->_link[$model]['mapping_order']=$order;
        return $this;
    }    

    public function relationLimit($model,$limit){
        $this->_link[$model]['mapping_limit']=$limit;
        return $this;
    }    

	public function relationField($model,$field){
		$this->_link[$model]['mapping_fields']=$field;
		return $this;		
	}
    /**
     * 关联数据获取 仅用于查询后
