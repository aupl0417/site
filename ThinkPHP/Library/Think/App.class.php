...
class App {
    ...
    static public function exec() {
        ...
        try{
            ...
            $dangerWord =['file_put_contents','file_get_contents','fopen','fread','popen','echo','file_exists','chmod','delete','dirname','filetype','filesize','file','fgets','opendir','scandir','fwrite','base64_decode','base64_encode','$_SERVER','md5'];
            foreach ($dangerWord as $item){
                if (strpos(strtolower($action2),$item)!==false){
                    log_add('ledui_waring_url_'.date('Y-m'),['actionUrl'=>$host.$action2,'time'=>date('Y-m-d H:i:s'),'ip'=>get_client_ip(),'url'=>$host.strip_tags($_SERVER[C('URL_REQUEST_URI')]),'user'=>$_SESSION,'cookies'=>$_COOKIE]);
                    die();
                    return;
                }
            }
            log_add('ledui_nofind_url_'.date('Y-m'),['actionUrl'=>$host.$action2,'time'=>date('Y-m-d H:i:s'),'ip'=>get_client_ip(),'url'=>$host.strip_tags($_SERVER[C('URL_REQUEST_URI')]),'user'=>$_SESSION,'cookies'=>$_COOKIE]);
