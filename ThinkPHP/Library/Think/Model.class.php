...
class Model {
    ...
    public function save($data='',$options=array()) {
        ...
        if(!isset($options['where']) ) {
            ...
            if (is_string($pk) && isset($data[$pk])) {
                ...
                // 增加复合主键支持
                foreach ($pk as $key=>$field) {
                    if($key==0){    //当使用键进更新数据会出错，所以只使用第一个主键作为条件　by enhong更新
                        if(isset($data[$field])) {
    ...
    public function autoCheckToken($data) {
        ...
        if(C('TOKEN_ON')){
			...
			*/
			...
			/**
			* by enhong 改写 2016-03-24 支持同页面多表单令牌验证
			*/
			$cache_name='form_token_'.session_id();
			$token=S($cache_name);
			//dump($token);
			$data[$name]=isset($data[$name])?$data[$name]:$_POST[$name];
			if(!isset($data[$name])){ // 令牌数据无效
                return false;
			}elseif(in_array($data[$name],$token)){
				//验证完成销毁
				$index=array_search($data[$name],$token);
				array_splice($token,$index,1);
				S($cache_name,$token);
				//dump($token);
				return true;
			}
			...
			// 开启TOKEN重置
			if(C('TOKEN_RESET')) S($cache_name,null);			
            return false;
        }
        return true;
    }
    ...
    protected function _validationFieldItem($data,$val) {
        switch(strtolower(trim($val[4]))) {
            ...
            case 'callback':// 调用方法进行验证                
                if(isset($val[6]) || is_array($val[6])) $args=array($val[6]);
                else $args = isset($val[6])?(array)$val[6]:array();
    ...
    public function db($linkNum='',$config='',$force=false) {
		...
		if(!is_null($config['DB_NAME'])) $this->dbName=$config['DB_NAME'];
        $this->_after_db();
        // 字段检测
