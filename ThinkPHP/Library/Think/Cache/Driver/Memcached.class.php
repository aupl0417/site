...
// p($serverList);
    ...
    public function getAllKeys(){
        $keys=array();
        foreach($this->handler->getAllKeys() as $val){
            if(substr($val,0,strlen($this->options['prefix'])) == $this->options['prefix']) $keys[] = $val;
        }
        return $keys;
    }
    ...
    public function clearAllKeys(){
        $keys = $this->handler->getAllKeys();
        foreach($keys as $val){
            //验前缀防止与其它项目前缀冲突
            if(substr($val,0,strlen($this->options['prefix'])) == $this->options['prefix']) $this->handler->delete($val);
        }
    }
