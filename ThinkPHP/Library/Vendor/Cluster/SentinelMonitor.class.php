...
class SentinelMonitor {
	...
	private function _getSlaveByMaster($masterName, $filter) {
        ...
        if (!empty($filter)) {
            foreach ($slaveName as $k => $val) {
                if ($val == $filter) {
                    unset($slaveName[$k]);
                }
            }
            $slaveName = array_values($slaveName);
        }
	...
	public function addServers($servers = array()) {
		...
		if (true === is_array($servers)) {
			foreach ($servers as $val) {
                $this->_servers[] = array('host' => $val['host'], 'port' => $val['port'], 'auth' => $val['auth'], 'db' => $val['db'], 'retry' => $val['retry']);
//				for ($i = 0; $i < $val[2]; $i++) {
...
//				}
	...
	public function getSlaveServer($masterName = '', $filter = '') {
		try {
			if (true === empty($masterName)) {
				$this->_connect();
				$masterName = $this->_getMaster();
				$slave = $this->_getSlaveByMaster($masterName, $filter);
				$slave = explode(':', $slave);
