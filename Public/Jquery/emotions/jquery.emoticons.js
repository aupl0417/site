            ...
            return this.each(function() {
                ...
                $(Obj).bind("click", function(e) {
                    ...
                    for( i = 0; i < options.emotions.length; i++) {
                        faceHtml += '<li text=' + options.emotions[i].phrase + ' type=' + i + '><img title=' + options.emotions[i].phrase + ' src="/Public/Jquery/emotions/emotions/'+ options.emotions[i].url + '"  style="cursor:pointer; position:relative;"   /></li>';
                    }
                    faceHtml += '</ul></div>';
                ...
                if(rs) {
                    for( i = 0; i < rs.length; i++) {
                        for( n = 0; n < em.length; n++) {
                            if(em[n].phrase == rs[i]) {
                                var t = "<img src='/Public/Jquery/emotions/emotions/"  + em[n].url + "' />";
...
//alert($(".emoticons").html());
//$(".emoticons").jqfaceedit({txtAreaObj:$("#content"),containerObj:$('.emoticons-box'),top:0,left:-100});